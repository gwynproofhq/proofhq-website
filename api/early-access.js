...
export default async function handler(req, res) {
  ...
  try {
    const { name, email, company, role, focus, website } = req.body
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Walkthrough request: ${name} at ${company || 'unknown'}`,
        html: `
          ...
          <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px;">
            ...
            <tr><td style="padding:6px 12px;font-weight:bold;">Focus</td><td style="padding:6px 12px;">${focus || '—'}</td></tr>
          </table>
          <p style="margin-top:16px;font-size:12px;color:#888;">Submitted via proofhq.io</p>
        `,
      }),
    })
